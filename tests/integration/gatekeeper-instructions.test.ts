...
async function main() {
...

  {
    ...
    assert.equal(
      Buffer.from(cofreAccount.viewKeyPublic).equals(Buffer.from(fixture.viewKeyPublic)),
      true,
    );
...

  {
    ...
    assert.equal(
      Buffer.from(licenseAccount.payloadHash).equals(Buffer.from(fixture.payloadHash)),
      true,
    );
...

  {
    ...
    const signerEntry = distAfterAdd.entries[0];
    assert.ok(signerEntry);
    assert.equal(signerEntry.signer.toBase58(), signer.toBase58());
    assert.equal(Buffer.from(signerEntry.ephemeralPk).equals(Buffer.from(ephemeralPk)), true);
    assert.equal(Buffer.from(signerEntry.nonce).equals(Buffer.from(viewNonce)), true);
    assert.equal(Buffer.from(signerEntry.ciphertext).equals(Buffer.from(ciphertext)), true);
    assert.equal(signerEntry.addedAt > 0n, true);
...

  {
    ...
    const revokedEntry = cofre.revokedAudit[0];
    assert.ok(revokedEntry);
    assert.equal(Buffer.from(revokedEntry).equals(Buffer.from(diversifier)), true);
...

  {
    ...
    const fullRevocations = Array.from({ length: MAX_REVOKED }, (_, index) =>
      repeated(16, index % 255),
    );
    context.setAccount(
      fixture.cofre,
      accountInfo(
