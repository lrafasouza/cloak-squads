...
| CPI target is the expected program | On-chain: `execute_with_license` validates CPI target against configured program ID (real Cloak on devnet) |
...
|------|---------|-----------|
| UTXO secrets (keypair, blinding) | `sessionStorage` only | Required for commitment recompute; never leaves browser |
| `commitmentClaim` (full claim) | `sessionStorage` only | Contains secrets above; not sent to API |
| UTXO data for stealth | SQLite (server-side) | Required for `fullWithdraw` claim flow; encrypted at rest |
...
1. **Real Cloak proofs** — `cloakDeposit()` calls `transact()` from the real Cloak SDK. Proofs are generated by the relay service and verified on-chain by the Cloak devnet program.
2. **Configurable CPI target** — `CLOAK_PROGRAM_ID` is loaded from env vars (`NEXT_PUBLIC_CLOAK_PROGRAM_ID`). The gatekeeper checks CPI target against this value at runtime.
3. **Rate limiting** — Implemented via `checkRateLimit()` on all API routes (60 requests/minute per IP).
4. **Threshold 1 only** — The UI handles display for multi-member but the tested flow is 1-of-1.
...
- [x] Replace `cloak-mock` with real Cloak program for proof verification
- [x] Make CPI target configurable (not hardcoded)
- [x] Add compute budget instructions (done: 1.4M CU + priority fee)
...
- ~~Real Cloak SDK proofs on devnet — SDK `deposit()` is broken~~ **RESOLVED** — Using `transact()` directly with zero inputs (deposit pattern)
...
| Wrong CPI target | Program ID check in execute_with_license | **Enforced** (configurable via env) |
| Front-running | License tied to specific operator wallet | **Mitigated** — operator is explicit |
| Fake proof submission | Real Cloak verifies Groth16 proof on-chain | **Enforced** — relay generates proofs |
| Rate limit bypass | 60 req/min per IP on all API routes | **Enforced** |
