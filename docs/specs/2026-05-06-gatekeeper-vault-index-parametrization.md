...
**Status:** Draft → não-implementado, deferido
...
### 2.2 Recomendação: parametrizar apenas runtime, manter admin em vault[0]
...
Reduz superfície de mudança de 8 → 2 handlers. Reduz risco de quebra de testes de integração de ~30 → ~10.

**Trade-off explícito:** `init_cofre` permanece Primary-only. Sub-vaults podem usar private send / payroll / invoice depois que a cofre estiver inicializada, mas a inicialização exige Primary. Isso já é o caso na prática (cofre init é parte do onboarding do vault).
