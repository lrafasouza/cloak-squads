...
- [x] **Audit fidelity pass (Sprint A, commit `176aaad`)** — payroll batches now expand to one row per recipient instead of collapsing to "payroll:N" (regulator-readable, no longer reads as structuring); cluster filter applied at every audit query so devnet rows can never leak into a mainnet export; `VaultIncome`, `StealthInvoice`, and `SwapDraft` joined into the audit feed for proof-of-reserves reconciliation; mock-data fallback removed from the public viewer (empty audits read as 0 transactions, not as 8 fabricated rows); single canonical endpoint `/api/audit/[linkId]/transactions` powers both the viewer and the signed export; `expiresAt` validated server-side (must be future, ≤365 d); `time_ranged` scope server-side requires both `startDate` and `endDate`; on-chain revocation cap (`Cofre.MAX_REVOKED = 256`) surfaced in the admin hero with warning at ≥80%
